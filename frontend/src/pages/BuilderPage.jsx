...
const schema = z.object({
  ...
  redirectLink: z.union([z.string().url("Must be a valid URL"), z.literal("")]),
  submissionCopyTo: z.union([z.string().email("Must be a valid email"), z.literal("")]),
  successMessage: z.string(),
}).superRefine((data, ctx) => {
  if (data.formType === "Leads Signup" && !data.venueName.trim()) {
    ctx.addIssue({ path: ["venueName"], code: z.ZodIssueCode.custom, message: "Venue Name is required for Leads Signup forms" });
  }
});
...
export default function BuilderPage() {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm({
    ...
    defaultValues: {
      title: "", formType: "Standard Form", formStatus: "live",
      venueName: "", sourceGroup: "", sourceName: "",
      assignLeadsTo: "", redirectLink: "", submissionCopyTo: "", successMessage: "",
    },
  });
  ...
  useEffect(() => {
    if (!isNew) {
      api.get(`/forms/${identifier}`).then(({ data }) => {
        reset({
          title: data.title || "",
          formType: FORM_TYPES.includes(data.formType) ? data.formType : "Standard Form",
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.settingsGrid}>
        ...
        <div style={styles.settingsSection}>
          ...
          <div style={styles.field}>
            <label style={styles.label}>Form Name <span style={styles.required}>*</span></label>
            <input style={styles.input} placeholder="Enter form name..." {...register("title")} />
          ...
          {formType === "Leads Signup" && (
            <div style={styles.field}>
              <label style={styles.label}>Venue Name <span style={styles.required}>*</span></label>
              <input style={styles.input} placeholder="Enter venue name..." {...register("venueName")} />
              {errors.venueName && <p style={styles.fieldError}>{errors.venueName.message}</p>}
            </div>
          )}

          <div style={styles.field}>
            <label style={styles.label}>Source Group <span style={styles.required}>*</span></label>
            <input style={styles.input} placeholder="e.g. Social Media" {...register("sourceGroup")} />
            {errors.sourceGroup && <p style={styles.fieldError}>{errors.sourceGroup.message}</p>}
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Source Name <span style={styles.required}>*</span></label>
            <input style={styles.input} placeholder="e.g. Facebook Campaign" {...register("sourceName")} />
            {errors.sourceName && <p style={styles.fieldError}>{errors.sourceName.message}</p>}
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Assign New Leads To</label>
            <input style={styles.input} placeholder="Name or email..." {...register("assignLeadsTo")} />
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Redirect Link After Submission</label>
            <input style={styles.input} placeholder="https://..." {...register("redirectLink")} />
            {errors.redirectLink && <p style={styles.fieldError}>{errors.redirectLink.message}</p>}
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Send a Copy of Submissions To</label>
            <input style={styles.input} placeholder="email@example.com" {...register("submissionCopyTo")} />
            {errors.submissionCopyTo && <p style={styles.fieldError}>{errors.submissionCopyTo.message}</p>}
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Submission Success Message</label>
            <textarea style={{ ...styles.input, resize: "vertical", minHeight: "80px" }} placeholder="Thank you for your submission!" {...register("successMessage")} />
          </div>
        </div>
              ...
              <>
                ...
                {bgImagePreview && (
                  <div style={styles.previewRow}>
                    ...
                    <button style={styles.clearBtn} onClick={() => { setBgImageFile(null); setBgImagePreview(null); }}>
                      Remove
                    </button>
                  </div>
                )}
      ...
      <div style={styles.saveRow}>
        {serverError && <p style={{ ...styles.error, margin: 0 }}>{serverError}</p>}
        <button type="submit" style={styles.saveBtn}>Save Form</button>
      </div>
      </form>
    </div>
  );
...
const styles = {
  ...
  previewRow: { display: "flex", alignItems: "center", gap: "0.75rem", marginTop: "0.25rem" },
